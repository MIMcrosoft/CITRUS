class MenuDeroulantRecherche extends HTMLElement{
    constructor() {
    super();
  ...
  connectedCallback() {
    ...
    searchDiv.className = "select-search-bar";
    const searchInput = document.createElement("input");
    searchInput.type = "text";
    searchInput.placeholder = "Rechercher...";
    searchInput.className = "select-search-bar-input";
    ...
    noResultsDiv.addEventListener("click", () => {
      ...
      this.dispatchEvent(
          new CustomEvent("no-result-click", {
            ...
            detail: { search: searchInput.value }
          })
        );
    });
    ...
    searchInput.addEventListener("keyup", () => {
      ...
      let visibleCount = 0;
      optionsDiv.querySelectorAll(".select-option:not(.select-noResults)").forEach(opt => {
        if (opt.textContent.toUpperCase().includes(filter)) {
          opt.style.display = "";
          visibleCount++;
        } else {
